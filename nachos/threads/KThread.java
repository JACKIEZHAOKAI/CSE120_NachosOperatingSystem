...
public class KThread {
	 ...
	 */
	private int status = statusNew;			// every thread starts in new status
	...
	private boolean called = false;		// add a boolean to keep thread if this thread has been called

	private KThread parentThread = null;		//add a thread to represent its parent thread called join()
	...
	public static void finish() {
		...
		//####################################################
		// when a thread is terminated and if it has a parent thread called join() on in
		// resumes the parent thread by calling ready() to move the parent thread to readyQuue and let scheduler to handle it
		//####################################################
		if (currentThread().called == true && currentThread().parentThread != null){
			currentThread().parentThread.ready();		// call ready() to unblock the parent thread
		}
	...
	public static void yield() {
		...
		// ############# critical section #####################
		// safety access to readyQueue
		currentThread.ready();	// add curr thread to readyQueue

		runNextThread();		// run the first thread in readyQueue
		// ###########################################
	...
	public void ready() {
		...
		// each thread has its own status: running / waiting/ ready
		status = statusReady;
		if (this != idleThread)
	 ...
	 * PA 1 part 2
	 * ########################################################################
	 * Implement KThread.join , which synchronizes the calling thread with
	 * the completion of the called thread
	 *
	 * When B calls join on A, block B until A finish, and unblock B once A finish
	 * B is the current thread, obtain by calling currentThread()
	 * A is this thread
	 *
	 *
	 */
	public void join() {
		...
		//	A thread cannot join to itself. (The initial implementation already checks for this
		//	case and invokes Lib.assert when it happens. Keep this Lib.assert call in your
		//	code.)
		Lib.assertTrue(this != currentThread);

		//we should be able to put a call to KThread.yield (causing the scheduler to choose another thread to run)
		//anywhere in your code where interrupts are enabled, and your code should still be correct
		boolean status = Machine.interrupt().disable();		// disable interrupt

		//	Join can be called on a thread at most once. If thread B calls join on A , then
		//	it is an error for B or any other thread C to call join on A again. Assert on
		//	this error.
		Lib.assertTrue(this.called == false && this.parentThread == null);

		//If A has already finishes, then B returns immediately from join without waiting.
		if(this.status == statusFinished){
			// do nothing
		}
		else{//	If A has not finishes, then B waits inside of join until A finishes;
			//	when A finishes, it resumes B .

			//set called and parent thread
			this.called = true;
			this.parentThread = currentThread();

			this.parentThread.sleep();		// block the parent thread
			// parentThread will get evoked when thread A finish by calling finish()
		}
	...
	/**########################################################################
	 * below are testcase for PingTest (PA0)	and 	joinTest (PA1 part2)
	 * #######################################################################
	 * */
	...
	private static void joinTest1 () {
		KThread child1 = new KThread( new Runnable () {
			public void run() {
				System.out.println("I (heart) Nachos!  printed by child thread");
			}
		});
		child1.setName("child1").fork();		// fork() will start executing the child1 thread
	...
	private static void joinTest2(){
		...
		t1.join();		// expect to join t1 and print foo  far fie fum and then resumes to curr thread
	...
	private static class A implements Runnable {
		...
		public void run () {
			...
			t2.join ();			// expect to join thread t2 and print fie and then resumes to curr thread t1
