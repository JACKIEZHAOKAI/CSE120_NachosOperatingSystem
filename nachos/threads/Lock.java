...
public class Lock {
	...
	public void acquire() {
		...
		if (lockHolder != null) {
			waitQueue.waitForAccess(thread);	//put curr thread to lock queue
			KThread.sleep();					// block curr thread
		}
		else {
	...
	public void release() {
		...
		if ((lockHolder = waitQueue.nextThread()) != null)	// remove first thread from lock wait queue
			lockHolder.ready();								// activate the thread from blocked state to ready state
